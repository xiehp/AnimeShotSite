...
<div>
	...
	<div class="shotList-blockTitle">
		...
		<c:if test="${!empty animeEpisode.summary and shotInfoPage.number == 0}">
			<button style="display: none;" type="button" class="btn btn-rimary btn-xs" data-toggle="collapse" data-target="#episodeSmmary">
	...
	<div>
		<c:forEach items="${ shotInfoPage.content }" var="shotInfo">
			<div style="min-height: 100px;" class="listImg">
				<a href="${ctx}/mip/shot/view/${shotInfo.id}" title="<c:out value='${animeEpisode.fullName}' /> <c:out value='${shotInfo.formatedTimeChina}' />">
...
<div>
	<a class="btn btn-primary" href="${ctx}/mip/episode/list/${animeEpisode.animeInfoId}" title="<c:out value='${animeInfo.fullName}' /> <c:out value='${animeInfo.secondName}' />">
