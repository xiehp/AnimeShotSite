...
function updateEpisodeTitleAndSummary(animeInfoId, episodeInfoId, forceUpdate) {
	if (confirm("是否继续？")) {
		var param = {};
		param.animeInfoId = animeInfoId;
		param.episodeInfoId = episodeInfoId;
		param.forceUpdate = forceUpdate;
		$.homePost(MANAGE_URL_STR + "/animeEpisode/updateEpisodeTitleAndSummary", param, function(data) {
			if (data.success) {
				$.showMessageModal(data.message);
			} else {
				$.showMessageModal(data.message);
 ...
 * 事件跟踪统计
 */
function trackEvent(category, action, label, value, domId) {
	try {
		if (isDefinedVariable("_czc") && _czc != null) {
			if (domId != null) {
				_czc.push([ "_trackEvent", category, action, label, value, domId ]);
			} else {
