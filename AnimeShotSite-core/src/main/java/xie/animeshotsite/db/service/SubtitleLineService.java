...
import xie.module.language.translate.baidu.XBaiduTranslateUtils;
...
public class SubtitleLineService extends BaseService<SubtitleLine, String> {
	...
	public void saveSubtitleLine(SubtitleInfo subtitleInfo, Boolean forceDelete) {
		...
		try {
			// 创建字幕数据
			file = FilePathUtils.getCommonFile(subtitleInfo.getLocalRootPath(), subtitleInfo.getLocalDetailPath(), subtitleInfo.getLocalFileName());
	...
	public List<SubtitleLine> findByTimeRemoveDuplicate(String animeEpisodeId, List<String> showLanage, Long shotStartTime, Long shotEndTime) {
		...
		String lanStr = XStringUtils.arrayToString(showLanage);
		List<SubtitleLine> list = entityCache.get(animeEpisodeId + "_" + lanStr + "_" + shotStartTime + "_" + shotEndTime, () -> {
