...
public interface GifInfoDao extends BaseRepository<GifInfo, String> {
	 ...
	 * 获得某个时间戳之前的一条记录
	 */
	@Query(value = "from GifInfo where animeEpisodeId = ?1 and timeStamp = (select MAX(s.timeStamp) from GifInfo s where s.animeEpisodeId = ?1 and s.timeStamp < ?2)")
	GifInfo findPrevious(String animeInfoId, Long timeStamp);

	/**
	 * 获得某个时间戳之后的一条记录
	 */
	@Query(value = "from GifInfo where animeEpisodeId = ?1 and timeStamp = (select MIN(s.timeStamp) from GifInfo s where s.animeEpisodeId = ?1 and s.timeStamp > ?2)")
	GifInfo findNext(String animeInfoId, Long timeStamp);
