package xie.common.httpMessageConvert;
...
public class FormToEntityHttpMessageConverter extends
	...
	public FormToEntityHttpMessageConverter() {
		...
		objectMapper = XJsonUtil.getObjectMapper().copy();
	}
	 ...
	 * <p>
	 * Prefixing the JSON string in this manner is used to help prevent JSON Hijacking. The prefix renders the string syntactically invalid as a script so that it cannot be hijacked. This prefix does not affect the evaluation of JSON, but if JSON validation is performed on the string, the prefix
	...
	/**
	 * Whether to use the {@link org.codehaus.jackson.impl.DefaultPrettyPrinter} when writing JSON. This is a shortcut for setting up an {@code ObjectMapper} as follows:
	 * 
	 * <pre>
	...
	protected JavaType getJavaType(Type type, Class<?> contextClass) {
		if (contextClass != null) {
			return TypeFactory.defaultInstance().constructType(type, TypeFactory.rawClass(contextClass));
		} else {
	...
	/**
	 * set propertie which is a list,such as List<cat> cat input's name must be cat>id
	 * 
	 * @param result
	 * @param name
	 * @param value
	...
	/**
	 * set propertie which is an entity too such as userInfo.accout.createDate input's name must be account.createDate
	 * 
	 * @param result
	 * @param name
	 * @param value
	...
	private String changeStrToUTF8(String value) {
		...
		/*
		 * try { value = getChangeString(value); } catch (Exception e) {
		 * 
		 * }
		 */
		return value;
